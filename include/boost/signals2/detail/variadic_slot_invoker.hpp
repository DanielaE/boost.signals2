  ...
  namespace signals2
  {
    namespace detail
    {
      ...
      public:
        ...
        R operator()(Func &func, const std::tuple<Args...> & args, std::integral_constant<size_t, N>) const
        {
      ...
      private:
        template<typename Func, unsigned ... indices, typename ... Args>
          R m_invoke(Func &func, unsigned_meta_array<indices...>, const std::tuple<Args...> & args,
            typename std::enable_if_t<std::negation_v<std::is_void<typename Func::result_type>>> * = 0
          ) const
        {
        ...
        template<typename Func, unsigned ... indices, typename ... Args>
          R m_invoke(Func &func, unsigned_meta_array<indices...>, const std::tuple<Args...> & args,
            typename std::enable_if_t<std::is_void_v<typename Func::result_type> > * = 0
          ) const
        {
      ...
      public:
          ...
          result_type operator ()(const ConnectionBodyType &connectionBody) const
        {
          return call_with_tuple_args<result_type>()(connectionBody->slot().slot_function(), 
            _args, std::integral_constant<size_t, sizeof...(Args)>());
        }
      private:
