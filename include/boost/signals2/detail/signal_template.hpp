  ...
  namespace signals2
  {
    namespace detail
    {
      template <typename Signature, typename C>
        requires std::is_same_v<Signature, std::remove_pointer_t<std::decay_t<C>>>
      bool operator==(const std::function<Signature> & Function,
                      const C & Callable) noexcept {
        auto pf = Function.template target<Signature *>();
        auto cf = reinterpret_cast<Signature *>(Callable);
        return pf && *pf == cf;
      }

      template <typename Signature>
      bool operator==(const std::function<Signature> & Function,
                      const std::function<Signature> & Callable) noexcept {
        auto pf = Function.template target<Signature *>();
        auto pc = Callable.template target<Signature *>();
        return pf && pc && *pf == *pc;
      }

      template <typename Signature, typename C>
      bool operator==(const std::function<Signature> & Function, const C & Callable) noexcept {
        return false;
      }
...
// signalN::slot_function
      ...
      public:
        ...
        {
          using is_group = std::is_convertible<T, group_type>;
      ...
      private:
        ...
        {
          BOOST_ASSERT(_shared_state.use_count() == 1);
          typename connection_list_type::iterator it;
          unsigned i;
          for(it = begin, i = 0;
        ...
        */
        ...
        void do_disconnect(const group_type &group, std::true_type /* is_group */)
        {
          disconnect(group);
        }
        template<typename T>
        void do_disconnect(const T &slot, std::false_type /* is_group */)
        {
          std::shared_ptr<invocation_state> local_state =
            get_readable_state();
          typename connection_list_type::iterator it;
    ...
    template <typename Signature> struct call_result;

    template <typename R, typename... Args>
    struct call_result<R(Args...)> {
      using type = R;
    };

    template<typename Signature, typename Combiner = optional_last_value<typename call_result<Signature>::type>, typename Group = int, typename GroupCompare = std::less<Group>, typename SlotFunction = std::function<Signature>, typename ExtendedSlotFunction = typename detail::variadic_extended_signature<Signature>::function_type, typename Mutex = signals2::mutex>
      class signal;
    ...
    public:
      typedef detail::weak_signal
        <R (Args...), Combiner, Group, GroupCompare, SlotFunction, ExtendedSlotFunction, Mutex> weak_signal_type;
      ...
      typedef typename impl_class::slot_call_iterator
        slot_call_iterator;
      typedef std::type_identity_t<R (Args...)> signature_type;
